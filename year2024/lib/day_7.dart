...
int testLine(
  ...
  final permutations = pow(2, values.length - 1);
  for (var p = 0; p < permutations; p++) {
    final binary = p.toRadixString(2).padLeft(values.length - 1, '0').split('');
    final operators =
        List.generate(values.length - 1, (i) => binary[i] == '1' ? mul : add);
